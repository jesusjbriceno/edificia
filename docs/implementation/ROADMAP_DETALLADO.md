...
| **7.3** | feature/editor-shell-refactor ✅ | • N/A | • Descomponer `EditorShell.tsx` en subcomponentes: `SyncBadge` (indicador de guardado), `EditorContextBar` (barra de contexto con metadatos del proyecto), `EditorEmptyState` (pantalla vacía). • Extraer lógica de acciones en hook `useEditorActions`. • Mejoras de accesibilidad (a11y): `aria-label`, `role`, contraste de botones, `cursor-pointer`. • Actualizar tests de integración para los nuevos subcomponentes. |
| **7.4** | feature/sidebar-completion-indicator ✅ | • N/A | • Añadir utilidades `hasContent()` (detecta contenido real en HTML de TipTap, maneja `<p></p>` y `<p><br></p>`) y `getLeafStats()` (agrega estadísticas hoja-completado recursivamente) en `src/lib/contentUtils.ts`. • `SidebarNavigation` lee `content` del store Zustand y lo pasa a `TreeNode`. • **Nodos hoja:** icono `CheckCircle2` (emerald) cuando completado, `FileText` cuando vacío; texto `text-white/80` vs `text-gray-400`. • **Nodos padre:** barra de progreso `h-px` (emerald si todo completo, brand-primary/50 si parcial) + contador `X/Y` en `text-[9px]`. • Tests: `contentUtils.test.ts` (17 casos), `SidebarNavigation.test.tsx` (18 casos), total 35 tests nuevos. |
