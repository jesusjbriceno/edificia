...
*   **Motor de Normativa y Editor**:
    ...
    *   `SidebarNavigation` (componente React) presenta el árbol de capítulos del CTE de forma recursiva, aplicando una función `filterTree(nodes, config)` para ocultar ramas (`requiresNewWork: true`) si el `InterventionType` es `Reform` o si `IsLoeRequired` es `false`. Incluye una **búsqueda recursiva en tiempo real** (`searchTree`) de capítulos. El panel incorpora además **indicadores visuales de completado**: los nodos hoja muestran un icono `CheckCircle2` (color esmeralda) cuando tienen contenido real, y los nodos padre muestran una barra de progreso `h-px` con un contador `X/Y` que refleja el porcentaje de subapartados cumplimentados. La lógica se apoya en las utilidades puras `hasContent()` —que descarta estados vacíos de TipTap como `<p></p>` o `<p><br></p>`— y `getLeafStats()` —que agrega recursivamente las estadísticas de hojas completadas—, ambas en `src/lib/contentUtils.ts` y cubiertas con 17 tests unitarios.
    *   `EditorHeader` proporciona un contexto multi-nivel (nombre del proyecto, tipo de intervención) y opciones de navegación.
    *   El contenido de la memoria se edita con **TipTap** en `EditorShell`, con una `EditorToolbar` para formato básico (negrita, cursiva, encabezados, listas). El módulo del editor se estructura siguiendo el principio de separación de responsabilidades: `EditorShell` actúa como orquestador, `EditorContextBar` encapsula la barra de acciones (breadcrumbs, estado de sincronización, botones Enviar a Revisión / Asistente IA / Exportar), `SyncBadge` refleja los 6 estados posibles de sync, `EditorEmptyState` gestiona el estado vacío, y el hook `useEditorActions` (`src/lib/hooks/`) centraliza la lógica de exportación, envío a revisión e inserción de contenido IA. Los botones de acción cumplen nivel AA de WCAG: `aria-pressed` en el toggle de IA, `aria-busy` en estados de carga, textos `sr-only` para lectores de pantalla, y contrastes revisados ("Enviar a Revisión": fondo sólido `emerald-500` + texto blanco; "Asistente IA": `text-blue-300`, ratio ≈7.5:1; "Borrador Local": `text-gray-300`).
